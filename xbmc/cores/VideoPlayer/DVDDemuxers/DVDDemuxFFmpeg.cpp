...
bool AttachmentIsFont(const AVDictionaryEntry* dict)
{
  if (dict)
  {
    ...
    return std::find_if(font_mimetypes.begin(), font_mimetypes.end(),
                        [&mimeType](std::string str)
                        { return str == mimeType; }) != font_mimetypes.end();
  }
  return false;
}
} // namespace
...
bool CDVDDemuxFFmpeg::Aborted()
{
  ...
  std::shared_ptr<CDVDInputStreamFFmpeg> input =
...
bool CDVDDemuxFFmpeg::Open(const std::shared_ptr<CDVDInputStream>& pInput, bool fileinfo)
{
  ...
  if (m_pInput->IsStreamType(DVDSTREAM_TYPE_FFMPEG))
  {
    ...
    else if (url.IsProtocol("udp") || url.IsProtocol("rtp"))
    {
      ...
      CLog::Log(LOGDEBUG, "CDVDDemuxFFmpeg::Open() UDP/RTP Original URL '%s'", strURL.c_str());
      ...
      if (found != std::string::npos)
      {
        ...
        if (found != std::string::npos && found > start)
        {
          ...
          strFile += "?sources=";
          strFile += strSourceIp;
          CLog::Log(LOGDEBUG, "CDVDDemuxFFmpeg::Open() UDP/RTP URL '%s'", strFile.c_str());
        }
      }
    }
    if (result < 0)
    {
      ...
      if (avformat_open_input(&m_pFormatContext, strFile.c_str(), iformat, &options) < 0)
      {
        CLog::Log(LOGDEBUG, "Error, could not open file (2) %s",
  ...
  else
  {
    ...
    m_ioContext =
        avio_alloc_context(buffer, bufferSize, 0, this, dvd_file_read, NULL, dvd_file_seek);
    ...
    if (iformat == nullptr)
    {
      ...
      if (trySPDIFonly || (iformat && strcmp(iformat->name, "wav") == 0))
      {
        ...
        std::unique_ptr<uint8_t[]> probe_buffer(
            new uint8_t[probeBufferSize + AVPROBE_PADDING_SIZE]);
        ...
        if (pd.buf_size <= 0)
        {
          CLog::Log(LOGERROR, "%s - error reading from input stream, %s", __FUNCTION__,
                    CURL::GetRedacted(strFile).c_str());
          return false;
        }
      ...
      if (!iformat)
      {
        CLog::Log(LOGERROR, "%s - error probing input format, %s", __FUNCTION__,
                  CURL::GetRedacted(strFile).c_str());
        return false;
      }
      else
    ...
    {
      string ext = strFile.substr(strFile.find_last_of(".") + 1);
      iformat = av_find_input_format("mp4");
      ...
      else if (ext == "enc3")
      {
        ...
        strFile.insert(0, "crypto:");
         ...
         iformat = av_find_input_format("wma");
      }
      else if (ext == "aac") {
         iformat = av_find_input_format("aac");
      }
      else if (ext == "m4a") {
        iformat = av_find_input_format("m4a");
      }
    }

    if (avformat_open_input(&m_pFormatContext, strFile.c_str(), iformat, &options) < 0)
    {
      CLog::Log(LOGERROR, "%s - Error, could not open file %s", __FUNCTION__,
                CURL::GetRedacted(strFile).c_str());
      Dispose();
      av_dict_free(&options);
      return false;
  ...
  if (m_streaminfo)
  {
    ...
    if (iErr < 0)
    {
      CLog::Log(LOGWARNING, "could not find codec parameters for %s",
                CURL::GetRedacted(strFile).c_str());
      if (m_pInput->IsStreamType(DVDSTREAM_TYPE_DVD) ||
  ...
  if (!skipCreateStreams || m_pFormatContext->nb_programs > 0)
  {
    ...
    if (m_pFormatContext->nb_programs > 0)
    {
      ...
      if (m_initialProgramNumber != UINT_MAX)
      {
        ...
        {
          if (m_pFormatContext->programs[i]->program_num ==
              static_cast<int>(m_initialProgramNumber))
          {
            nProgram = i;
            m_initialProgramNumber = UINT_MAX;
            break;
...
void CDVDDemuxFFmpeg::Dispose()
{
  ...
  if (m_pFormatContext)
  {
    ...
    {
      CLog::Log(LOGWARNING, "CDVDDemuxFFmpeg::Dispose - demuxer changed our byte context behind "
                            "our back, possible memleak");
...
AVDictionary* CDVDDemuxFFmpeg::GetFFMpegOptionsFromInput()
{
  ...
  if (url.IsProtocol("http") || url.IsProtocol("https"))
  {
    ...
    for (std::map<std::string, std::string>::const_iterator it = protocolOptions.begin();
         it != protocolOptions.end(); ++it)
    {
      ...
      if (name == "seekable" || name == "reconnect" || name == "reconnect_at_eof" ||
          name == "reconnect_streamed" || name == "reconnect_delay_max" || name == "icy" ||
          name == "icy_metadata_headers" || name == "icy_metadata_packet")
      {
        CLog::Log(LOGDEBUG,
      ...
      else if (name == "user-agent")
      {
        ...
        CLog::Log(
            LOGDEBUG,
            "CDVDDemuxFFmpeg::GetFFMpegOptionsFromInput() adding ffmpeg option 'user_agent: %s'",
            value.c_str());
        hasUserAgent = true;
      }
      else if (name == "cookies")
      {
        ...
        CLog::Log(LOGDEBUG,
                  "CDVDDemuxFFmpeg::GetFFMpegOptionsFromInput() adding ffmpeg option 'cookies: %s'",
                  value.c_str());
        hasCookies = true;
      }
      else if (name == "cookie")
      {
        CLog::Log(
            LOGDEBUG,
            "CDVDDemuxFFmpeg::GetFFMpegOptionsFromInput() adding ffmpeg header value 'cookie: %s'",
            value.c_str());
      ...
      // other standard headers (see https://en.wikipedia.org/wiki/List_of_HTTP_header_fields) are appended as actual headers
               ...
               name == "content-md5" || name == "date" || name == "expect" || name == "forwarded" ||
               name == "from" || name == "if-match" || name == "if-modified-since" ||
               name == "if-none-match" || name == "if-range" || name == "if-unmodified-since" ||
               name == "max-forwards" || name == "origin" || name == "pragma" || name == "range" ||
               name == "referer" || name == "te" || name == "upgrade" || name == "via" ||
               name == "warning" || name == "x-requested-with" || name == "dnt" ||
               name == "x-forwarded-for" || name == "x-forwarded-host" ||
               name == "x-forwarded-proto" || name == "front-end-https" ||
               name == "x-http-method-override" || name == "x-att-deviceid" ||
               name == "x-wap-profile" || name == "x-uidh" || name == "x-csrf-token" ||
               name == "x-request-id" || name == "x-correlation-id")
      {
        if (name == "authorization")
        {
          CLog::Log(LOGDEBUG,
                    "CDVDDemuxFFmpeg::GetFFMpegOptionsFromInput() adding custom header option '%s: "
                    "***********'",
                    it->first.c_str());
        }
        else
        {
          CLog::Log(
              LOGDEBUG,
              "CDVDDemuxFFmpeg::GetFFMpegOptionsFromInput() adding custom header option '%s: %s'",
              it->first.c_str(), value.c_str());
        }
        headers.append(it->first).append(": ").append(value).append("\r\n");
      }
    ...
    if (!hasUserAgent)
    {
      ...
      av_dict_set(
          &options, "user_agent",
          CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_userAgent.c_str(), 0);
    }
...
double CDVDDemuxFFmpeg::ConvertTimestamp(int64_t pts, int den, int num)
{
  ...
  std::shared_ptr<CDVDInputStream::IMenus> menu =
...
DemuxPacket* CDVDDemuxFFmpeg::Read()
{
  ...
  if (pPacket && pPacket->iStreamId >= 0)
  {
    ...
    if (!stream || stream->pPrivate != m_pFormatContext->streams[pPacket->iStreamId] ||
    ...
    else if (stream->type == STREAM_AUDIO)
    {
      ...
      if (audiostream && (audiostream->iChannels !=
                              m_pFormatContext->streams[pPacket->iStreamId]->codecpar->channels ||
    ...
    else if (stream->type == STREAM_VIDEO)
    {
      if (static_cast<CDemuxStreamVideo*>(stream)->iWidth !=
              m_pFormatContext->streams[pPacket->iStreamId]->codecpar->width ||
          static_cast<CDemuxStreamVideo*>(stream)->iHeight !=
              m_pFormatContext->streams[pPacket->iStreamId]->codecpar->height)
      {
        // content has changed
        stream = AddStream(pPacket->iStreamId);
      }
...
bool CDVDDemuxFFmpeg::SeekTime(double time, bool backwards, double* startpts)
{
  ...
  if (m_checkTransportStream)
  {
    ...
    while (!IsTransportStreamReady())
    {
      ...
      if (timer.IsTimePast())
      {
        CLog::Log(LOGERROR, "CDVDDemuxFFmpeg::%s - Timed out waiting for video to be ready",
                  __FUNCTION__);
        return false;
      }
    }
  ...
  int ret;
  {
    ...
    if (ret < 0)
    {
      ...
      if (m_pFormatContext->duration && seek_pts >= (m_pFormatContext->duration + starttime))
      {
        // force eof
...
int CDVDDemuxFFmpeg::GetStreamLength()
{
  ...
  if (m_pFormatContext->duration < 0 || m_pFormatContext->duration == AV_NOPTS_VALUE)
    return 0;
...
int CDVDDemuxFFmpeg::GetPrograms(std::vector<ProgramInfo>& programs)
{
  ...
  for (unsigned int i = 0; i < m_pFormatContext->nb_programs; i++)
  {
    ...
    AVDictionaryEntry* tag =
        av_dict_get(m_pFormatContext->programs[i]->metadata, "", nullptr, AV_DICT_IGNORE_SUFFIX);
    while (tag)
    {
  ...
  if (pStream && pStream->discard != AVDISCARD_ALL)
  {
    ...
    switch (pStream->codecpar->codec_type)
    {
      ...
      case AVMEDIA_TYPE_VIDEO:
      {
        ...
        if (pStream->codec_info_nb_frames > 0 && pStream->codec_info_nb_frames <= 2 &&
            m_pInput->IsStreamType(DVDSTREAM_TYPE_DVD))
        {
          CLog::Log(LOGDEBUG, "%s - fps may be unreliable since ffmpeg decoded only %d frame(s)",
        ...
        if (m_pInput->IsStreamType(DVDSTREAM_TYPE_DVD))
        {
          if (pStream->codecpar->codec_id == AV_CODEC_ID_PROBE)
          {
            ...
            if ((pStream->id & 0xF0) == 0xE0)
            {
              ...
              CLog::Log(LOGERROR, "%s - AV_CODEC_ID_PROBE detected, forcing AV_CODEC_ID_MPEG2VIDEO",
                        __FUNCTION__);
            }
          }
        }
      ...
      {
        if (pStream->codecpar->codec_id == AV_CODEC_ID_DVB_TELETEXT &&
            CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(
                CSettings::SETTING_VIDEOPLAYER_TELETEXTENABLED))
        {
          ...
          else
          {
            ...
            if (pStream->codecpar->extradata && file.OpenForWrite(fileName))
            {
                  ...
                  pStream->codecpar->extradata_size)
              {
                ...
                CLog::Log(LOGDEBUG, "%s: Error saving font file \"%s\"", __FUNCTION__,
                          fileName.c_str());
              }
            }
          }
      ...
      default:
      {
        ...
        {
          CLog::Log(LOGDEBUG, "CDVDDemuxFFmpeg::AddStream - discarding unknown stream with id: %d",
                    pStream->index);
          pStream->discard = AVDISCARD_ALL;
          return nullptr;
        }
    ...
    if (stream->type != STREAM_NONE && pStream->codecpar->extradata &&
    ...
    if (m_pInput->IsStreamType(DVDSTREAM_TYPE_BLURAY))
    {
      ...
      auto it = std::find_if(m_streams.begin(), m_streams.end(),
                             [&stream](const std::pair<int, CDemuxStream*>& v) {
                               return (v.second->dvdNavId == stream->dvdNavId) &&
                                      (v.second->type == stream->type);
                             });
      ...
      if (it != m_streams.end())
      {
        if (stream->codec == AV_CODEC_ID_AC3 && it->second->codec == AV_CODEC_ID_TRUEHD)
          CLog::Log(
              LOGDEBUG,
              "CDVDDemuxFFmpeg::AddStream - discarding duplicated bluray stream (truehd ac3 core)");
        else
          CLog::Log(LOGDEBUG, "CDVDDemuxFFmpeg::AddStream - discarding duplicate bluray stream %s",
                    stream->codecName);
...
int CDVDDemuxFFmpeg::GetChapter()
{
  ...
  for (unsigned i = 0; i < m_pFormatContext->nb_chapters; i++)
  {
    ...
    if (m_currentPts >=
        ...
        m_currentPts <
            ConvertTimestamp(chapter->end, chapter->time_base.den, chapter->time_base.num))
      return i + 1;
  }
...
void CDVDDemuxFFmpeg::GetChapterName(std::string& strChapterName, int chapterIdx)
{
  ...
  else
  {
    ...
    AVDictionaryEntry* titleTag =
        av_dict_get(m_pFormatContext->chapters[chapterIdx - 1]->metadata, "title", NULL, 0);
    if (titleTag)
...
int64_t CDVDDemuxFFmpeg::GetChapterPos(int chapterIdx)
{
  ...
  return static_cast<int64_t>(m_pFormatContext->chapters[chapterIdx - 1]->start *
...
bool CDVDDemuxFFmpeg::IsProgramChange()
{
  ...
  for (unsigned int i = 0; i < m_pFormatContext->programs[m_program]->nb_stream_indexes; i++)
  {
    ...
    if (m_pFormatContext->streams[idx]->codecpar->extradata_size !=
        static_cast<int>(stream->ExtraSize))
      return true;
  }
  return false;
}
...
unsigned int CDVDDemuxFFmpeg::HLSSelectProgram()
{
  ...
  int bandwidth = CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(
                      CSettings::SETTING_NETWORK_BANDWIDTH) *
                  1000;
  if (bandwidth <= 0)
    bandwidth = INT_MAX;
  ...
  for (unsigned int i = 0; i < m_pFormatContext->nb_programs; ++i)
  {
    int strBitrate = 0;
    AVDictionaryEntry* tag =
        av_dict_get(m_pFormatContext->programs[i]->metadata, "variant_bitrate", NULL, 0);
    if (tag)
    ...
    for (unsigned int j = 0; j < m_pFormatContext->programs[i]->nb_stream_indexes; j++)
    {
      ...
      if (pStream && pStream->codecpar && pStream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO)
      {
...
void CDVDDemuxFFmpeg::ParsePacket(AVPacket* pkt)
{
  ...
  if (st && st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO)
  {
    ...
    {
      m_parsers.insert(
          std::make_pair(st->index, std::unique_ptr<CDemuxParserFFmpeg>(new CDemuxParserFFmpeg())));
    ...
    if (parser->second->m_parserCtx && parser->second->m_parserCtx->parser &&
        parser->second->m_parserCtx->parser->split && !st->codecpar->extradata)
    {
      int i = parser->second->m_parserCtx->parser->split(parser->second->m_codecCtx, pkt->data,
                                                         pkt->size);
      if (i > 0 && i < FF_MAX_EXTRADATA_SIZE)
      {
        ...
        {
          CLog::Log(LOGDEBUG,
                    "CDVDDemuxFFmpeg::ParsePacket() fetching extradata, extradata_size(%d)", i);
          ...
          if (parser->second->m_parserCtx->parser->parser_parse)
          {
            ...
            parser->second->m_parserCtx->parser->parser_parse(
                parser->second->m_parserCtx, parser->second->m_codecCtx, &outbufptr, &bufSize,
                pkt->data, pkt->size);
        ...
        else if (content.compare(pos, 5, "rate=", 5) == 0)
        {
          ...
          if (!rate.empty())
          {
            ...
            if (val > 0)
              ...
              CLog::Log(LOGDEBUG, "CDVDDemuxFFmpeg::%s - no parameter for samplerate",
                        __FUNCTION__);
          }
        }
